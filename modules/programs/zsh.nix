      ...
      {config, ...}: {
        options = {
          ...
          path = mkOption {
            type = types.str;
            default =
              if lib.versionAtLeast stateVersion "20.03"
          ...
          programs.zsh.initContent = lib.mkMerge [
            ...
            (mkOrder 900 (
              ...
              + ''
                ...
                ${
                  if cfg.history.ignoreAllDups
                  then "setopt"
                  else "unsetopt"
                } HIST_IGNORE_ALL_DUPS
                ${
                  if cfg.history.saveNoDups
                  then "setopt"
                  else "unsetopt"
                } HIST_SAVE_NO_DUPS
                ${
                  if cfg.history.findNoDups
                  then "setopt"
                  else "unsetopt"
                } HIST_FIND_NO_DUPS
                ${
                  if cfg.history.ignoreSpace
                  then "setopt"
                  else "unsetopt"
                } HIST_IGNORE_SPACE
                ${
                  if cfg.history.expireDuplicatesFirst
                  then "setopt"
                  else "unsetopt"
